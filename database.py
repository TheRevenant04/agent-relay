...
engine configuration.  The rest of the application talks to the models through
:mod:`storage`.  Timestamps are stored as timezone-aware ``timestamptz``
values in UTC, and every write that must be race-safe takes explicit row locks
via ``FOR UPDATE`` / ``FOR UPDATE SKIP LOCKED``.
"""
...
def _database_url() -> str:
    return (
        ...
        or "postgresql+psycopg://relay:relay@localhost:5432/agent_relay"
    )
...
def as_db_time(value: datetime) -> datetime:
    """PostgreSQL's ``timestamptz`` column stores aware, UTC-normalized values."""
...
def recover_expired_in_session(db: Session, now: datetime) -> int:
    ...
    The expired attempts are locked with ``FOR UPDATE`` so recovery processes
    cannot race each other or a heartbeat/terminal submission over the same
    attempt row.
    """
