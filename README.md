...
The quickest path runs Agent Relay and PostgreSQL together with Compose:

```bash
docker compose up --build
```

That publishes the API on <http://127.0.0.1:8000/> and PostgreSQL on
`localhost:5432`. Open <http://127.0.0.1:8000/> for the token-based local
dashboard. Set `RELAY_DATABASE_URL` to point at another PostgreSQL server
(default `postgresql+psycopg://relay:relay@localhost:5432/agent_relay`).
...
To run the API directly against a PostgreSQL you manage:
...
`database.py` contains SQLAlchemy models and the PostgreSQL engine setup.
`storage.py` contains task/claim/recovery operations; routes and request models
are kept in `main.py` and `schemas.py`. Concurrent claims are safe across
processes using PostgreSQL row locking (`FOR UPDATE SKIP LOCKED`), so two
workers cannot hold the same task while its lease is active. Timestamps are
stored as timezone-aware `timestamptz` values in UTC.
...
Tests default to the scratch database `agent_relay_test` on
`localhost:5432`, which the compose `postgres` service provisions. Start it
first (`docker compose up -d postgres`) or set `RELAY_DATABASE_URL` to another
scratch database. The fixture drops and recreates all tables on whatever
`RELAY_DATABASE_URL` points at, so never point it at a database whose data you
need.
