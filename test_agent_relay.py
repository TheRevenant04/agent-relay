...
SKIP LOCKED`` on claim), not from a Python lock.
"""
...
# Default to the scratch database the compose file provisions for tests, so
# `pytest` never resets the dev server's database. Respect an explicit
# RELAY_DATABASE_URL/DATABASE_URL override.
